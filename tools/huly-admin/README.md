...
| `praut-purge-user.cjs` | **Okamžitý trvalý výmaz** zaměstnance (bez 60denní grace). Smaže `contact:class:Person` + jeho social identity; cílí VÝHRADNĚ podle person _id. Pojistky: odmítne osobu s přiřazenými úkoly / členstvím v prostoru / chráněné jméno. `--sql` vypíše i DB DELETE (social_id + person na serveru). DRY-RUN → `--apply`. |
